...
class CompressedDataReader:
    ...
    def decompress_window_data(compressed_data):
        ...
        try:
            ...
            print(f"decompress_window_data error: {e}")
...
def load_compressed_data_from_database(batch_size=1000):
    ...
    with conn.cursor(name='compressed_cursor') as cur:
        ...
        for row in tqdm(cur, total=total_windows, desc="解壓縮進度"):
            ...
            # 解壓縮
            x_data, y_data, z_data = reader.decompress_window_data(
            ...
            if batch_count % 1000 == 0:
                print(f"processed {batch_count}/{total_windows} windows")
    ...
    for label, count in zip(unique, counts):
        ...
        print(f" windows containing {activity_name}: {count}  ({count/len(y)*100:.1f}%)")
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig('training_history_compressed.png',
                dpi=300, bbox_inches='tight')
    print("training_history saved: training_history_compressed.png")
    plt.close()
...
def plot_confusion_matrix(y_true, y_pred, activity_labels):
    ...
    plt.tight_layout()
    plt.savefig('confusion_matrix_compressed.png',
                dpi=300, bbox_inches='tight')
    print("confusion_matrix saved: confusion_matrix_compressed.png")
    plt.close()
...
def main_standard():
    ...
    try:
        print("=" * 60)
        print("training HAR model from compressed database ")
        print("=" * 60)
        ...
        print("\nmodel saved: final_model_compressed.keras")
